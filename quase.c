...
void generate_strong_password(char *password);  // Função para gerar senha forte
void display_hash_info(void);  // Função para exibir informações do hash


volatile int good_password = 0;
volatile int button_p1_1_pressed = 0;
volatile int button_p2_1_pressed = 0;

void __attribute__ ((interrupt(PORT1_VECTOR))) Port_1(void) {
    if (P1IFG & BIT1) {
        button_p1_1_pressed = 1;
...
void generate_strong_password(char *password) {
    ...
    int i; // Declaração da variável 'i' fora do loop
    // Gerar 10 caracteres aleatórios da senha
    for (i = 0; i < 15; i++) {
        password[i] = chars[rand() % (sizeof(chars) - 1)];
    }
    // Adicionar caractere nulo ao final da senha
    password[15] = '\0';
}

void display_hash_info(void) {
    lcd_clear();
    lcd_str("Tipo: MD5");
    lcd_cursor(0x40);
...
int main(void){
    ...
    char text[] = "Str0ng!Passw0rdS"; // Texto para criptografar Str0ng!Passw0rdS seraque
    ...
    lcd_clear();
    lcd_str("1.1|Senha forte");
    lcd_cursor(0x40);
    lcd_str("2.1|Analise Hash");
    __delay_cycles(10000000);  // espera aproximadamente 1 min

    if (button_p1_1_pressed) {
        char strong_password[11];
        generate_strong_password(strong_password);
        lcd_clear();
        lcd_str("Senha Forte:");
        lcd_cursor(0x40);
        lcd_str(strong_password);
        __delay_cycles(10000000);
...
void gpio_config(void){
    ...
    P1IE |= BIT1;  // Habilitar interrupção no pino P1.1
    P1IFG &= ~BIT1; // Limpar flag de interrupção

    P2IE |= BIT1;  // Habilitar interrupção no pino P2.1
